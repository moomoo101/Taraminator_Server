...
if [ $? != 0 ]; then
	...
	if [ $? != 0 ]; then
		...
		sudo apt-get -y install libavformat-dev libavutil-dev libswscale-dev libglib2.0-dev
						...
						if [ $? != 0 ]; then
							...
							if [ $? != 0 ]; then
								...
								if [ $? != 0 ]; then
									...
									echo -e "${BLUE}${BOLD}Downloading OpenCV version 3.1.0\n${NC}"
									wget -O opencv-3.1.0.zip https://github.com/Itseez/opencv/archive/3.1.0.zip
									if [ $? != 0 ]; then
										echo -e "${RED}${BOLD}Make sure internet is connected${NC}"
										echo -e "${RED}${BOLD}Downloading OpenCV 3.1.0 failed${NC}"
										RET=1;
									else
										...
										if [ $? != 0 ]; then
											...
											echo -e "\n${BLUE}${BOLD}Replacing ${RED}cap_v4l.cpp ${BLUE}${BOLD}in videoio module with e-con's patch file to add Y16 format support\n${NC}"
											unzip -q opencv_contrib-3.1.0.zip 
											unzip -q opencv-3.1.0.zip
											cp ./../opencv-patch/cap_v4l.cpp opencv-3.1.0/modules/videoio/src
											if [ $? != 0 ]; then
												echo -e "${RED}${BOLD}Copying the opencv_patch failed${NC}"
												RET=1;
											else
												...
												if [ $? != 0 ]; then
													...
													echo -e "\n${GREEN}${BOLD}OpenCV 3.1.0 is built and ready to be used\n${NC}"
												fi
											fi
										fi
									fi
